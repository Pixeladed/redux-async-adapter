...
describe('selectAllStatuses', () => {
  it('should return empty array if no thunks have been dispatched', () => {
    ...
    const statuses = selectors.selectAllStatuses(store.getState());
  ...
  it('should return all statuses of dispatched thunks', async () => {
    ...
    const store = configureStore({
      reducer: createReducer(getInitialState({}), {
        [thunk1.pending.type]: handlePending(thunk1),
        [thunk1.fulfilled.type]: handleFulfilled(thunk1),
        [thunk1.rejected.type]: handleRejected(thunk1),
        [thunk2.pending.type]: handlePending(thunk2),
        [thunk2.fulfilled.type]: handleFulfilled(thunk2),
        [thunk2.rejected.type]: handleRejected(thunk2),
        [thunk3.pending.type]: handlePending(thunk3),
        [thunk3.fulfilled.type]: handleFulfilled(thunk3),
        [thunk3.rejected.type]: handleRejected(thunk3),
      }),
    });
